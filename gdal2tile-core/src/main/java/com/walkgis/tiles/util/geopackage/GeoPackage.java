...
public class GeoPackage {
    private static GeoPackage instance;
    ...
    private GeoPackage(String filename, String version, Map<String, String> options, boolean create) {
    ...
    public static GeoPackage create(String filename, Map<String, String> _metadata, String version) {
        if (version == null) {
            version = Metadata_1_2.VERSION;
        }

        instance = GeoPackage._create(filename, version);
        instance.metadata._setup(_metadata);
        return instance;
    }

    private static GeoPackage _create(String filename, String version) {
        File file = new File(filename);
        if (file.exists())
            file.delete();

        instance = new GeoPackage(filename, version, null, true);
